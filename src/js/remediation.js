  ...
  findLiteralPriceToken(item, rowEvidence) {
    ...
    if (rowEvidence && Array.isArray(rowEvidence.anchors)) {
      for (const a of rowEvidence.anchors) {
        candidates.push({
          str: (a && typeof a.rawLine === "string" ? a.rawLine : (a && a.str)) || "",
    ...
    for (const t of candidates) {
      ...
      const anchorDy =
        typeof t.y === "number" && typeof item.y === "number"
          ...
          : 0;
      if (
        Math.abs(dy) > ROW_TOLERANCE &&
        anchorDy > ROW_TOLERANCE
      )
        continue; // not in the row band
      if (
        anchorX !== null &&
        typeof t.x === "number" &&
      ...
      return { str, x: t.x, page: t.page || (rowEvidence && rowEvidence.page), dy };
    }
    return null;
  },
