...
const CatalogView = {
  ...
  renderCatalog() {
    ...
    pageItems.forEach(r => {
      ...
      const imgHtml = hasCatalogImage(r.img) ? `<img src="${esc(r.img)}" onerror="this.onerror=null; this.src='${DEFAULT_SVG_IMG}';" style="width: 32px; height: 32px; object-fit: contain; border-radius: 6px; cursor: zoom-in; background: rgba(0,0,0,0.4); border: 1px solid var(--border);" onclick="zoomImage('${skuJs}')" loading="lazy" decoding="async">` : `<img src="${DEFAULT_SVG_IMG}" alt="Sin imagen" title="Sin imagen: requiere revisión" loading="lazy" decoding="async" style="width: 32px; height: 32px; object-fit: contain; border-radius: 6px; opacity: 0.4;">`;
    ...
    if (CatalogView.catalogViewMode === 'grid') {
      ...
      pageItems.forEach(r => {
        ...
        gridHtml += `<img src="${esc(imgSrc)}" onerror="this.onerror=null; this.src='${DEFAULT_SVG_IMG}';" loading="lazy" decoding="async" style="max-width: 100%; max-height: 100%; object-fit: contain; image-rendering: -webkit-optimize-contrast;">`;
        gridHtml += `</div>`;
