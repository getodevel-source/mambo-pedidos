...
global.SpreadsheetHarness = require(
	path.join(__dirname, "quality", "spreadsheet-harness.js"),
);
global.UpdaterSmoke = require(
	path.join(__dirname, "quality", "updater-smoke.js"),
);
global.esc = (value) =>
	String(value || "")
		.replace(/&/g, "&amp;")
...
(function checkScriptIntegrity() {
	...
	const walk = (dir) => {
		for (const ent of _fs.readdirSync(dir, { withFileTypes: true })) {
			...
			if (ent.isDirectory()) walk(full);
				...
				if (
					!loaded.has(rel) &&
					!["tests.js", "catalogAssignmentGates.js"].includes(ent.name)
				)
					missing.push(rel);
			}
		}
	...
	if (missing.length) {
		console.error(
			"❌ Script integrity FAILED — archivos de app no cargados en index.html: " +
				missing.join(", "),
		);
		process.exitCode = 1;
	} else {
		console.log(
			"✅ Script integrity OK: todos los módulos de src/js están en index.html",
		);
	}
})();
...
(async () => {
	...
	(function checkDynamicRenderMethods() {
		const src = _fs.readFileSync(
			path.join(__dirname, "..", "src", "js", "ui", "importWizard.js"),
			"utf8",
		);
		const ids = [...src.matchAll(/id:\s*'([^']+)'/g)].map((m) => m[1]);
		const missing = ids.filter(
			(id) => !new RegExp(`_render_${id}\\s*\\(`).test(src),
		);
		if (missing.length) {
			console.error(
				"❌ Wizard dynamic render FAILED — faltan métodos _render_ para: " +
					missing.join(", "),
			);
			process.exitCode = 1;
		} else {
			console.log(
				`✅ Wizard dynamic render OK (${ids.length} pasos con su _render_*)`,
			);
		}
	})();
	...
	(function checkNoUnguardedProcessRefs() {
		...
		if (hits.length) {
			console.error(
				"❌ Browser-runtime FAILED — referencias a `process.` sin guarda (rompen en WebView2):",
			);
			hits.forEach((h) => console.error("   " + h));
			process.exitCode = 1;
		} else {
			console.log(
				"✅ Browser-runtime OK: src/js no referencia `process.` sin guarda",
			);
		}
	})();
	...
	try {
		...
		execFileSync(
			process.execPath,
			[path.join(__dirname, "quality", "ui-smoke-tests.js")],
			{ stdio: "inherit" },
		);
	} catch (uiErr) {
	...
	try {
		...
		execFileSync(
			process.execPath,
			[path.join(__dirname, "quality", "logic-tests.js")],
			{ stdio: "inherit" },
		);
	} catch (logicErr) {
	...
	try {
		...
		execFileSync(
			process.execPath,
			[path.join(__dirname, "quality", "app-smoke-tests.js")],
			{ stdio: "inherit" },
		);
	} catch (appErr) {
		console.error(
			"❌ app.js smoke tests FAILED: " + (appErr.message || appErr),
		);
		process.exitCode = 1;
	}
	...
	try {
		...
		execFileSync(
			process.execPath,
			[path.join(__dirname, "build-frontend.js")],
			{ stdio: "inherit" },
		);
	} catch (buildErr) {
		console.error(
			"❌ Build frontend FAILED: " + (buildErr.message || buildErr),
		);
		process.exitCode = 1;
	}
})().catch((error) => {
