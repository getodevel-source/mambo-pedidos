	...
	getImageData(x, y, w, h) {
		...
		for (let j = 0; j < h; j++) {
			for (let i = 0; i < w; i++) {
				...
				const d = (j * w + i) * 4;
				data[d] = this._data[s];
				data[d + 1] = this._data[s + 1];
				data[d + 2] = this._data[s + 2];
				data[d + 3] = this._data[s + 3];
			}
		}
...
PdfParser.extractImagesFromPage = async function (page, viewport, pageNum) {
	...
	imageStats.push({
		...
		imgs: imgs.map((i) => ({
			...
			w: i.width,
			h: i.height,
			aspect: Number((i.width / Math.max(1, i.height)).toFixed(2)),
		})),
	});
	return imgs;
};
...
(async () => {
	...
	const files = fs
		...
		.filter(
			(f) =>
				!process.env.CATALOG_FILTER || f.includes(process.env.CATALOG_FILTER),
		);
	console.log(
		`🔍 Reprocesando ${files.length} catálogos en ${CATALOG_DIR}...\n`,
	);
	...
	for (const fileName of files) {
		...
		try {
			...
			for (const p of products) {
				...
				if (!byPage.has(key))
					byPage.set(key, { page: p.pageNum || 0, prods: 0, noImg: 0 });
			...
			const exported = products.map((p) => ({
				...
				grounded: !!p.grounded,
				sourceFile: fileName,
				qualityReason: p.qualityReason || "Sin observaciones",
				pageNum: p.pageNum,
				x: p.x,
				y: p.y,
				...
				imgTextWarnings: Array.isArray(p._imgTextWarnings)
					? p._imgTextWarnings
					: undefined,
				imgAspect:
					typeof p._imgAspect === "number" && Number.isFinite(p._imgAspect)
						? p._imgAspect
						: undefined,
			...
			perFile.push({
				...
				placeholders: exported.filter((p) => p.img === "-").length,
			});
			console.log(
				`📄 [${fileName}] → ${brand} | ${exported.length} productos | ${perFile[perFile.length - 1].placeholders} sin imagen`,
			);
		} catch (err) {
...
🛡️ Gates: ${beforeGreen} GREEN → ${gated.stats.green} GREEN / ${gated.stats.yellow} YELLOW / ${gated.stats.red} RED (${gated.stats.total} productos)`);
	...
	console.log(
		"Por catálogo:",
		perFile.map((f) => `${f.file.split(" ")[0]}:${f.count}`).join(" "),
	);
})().catch((err) => {
	console.error(err);
	process.exit(1);
