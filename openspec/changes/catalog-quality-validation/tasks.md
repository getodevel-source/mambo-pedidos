...
- [ ] 3.1 **RED** Test: physical CSV/XLSX catalog via `AiCatalogEngine` preserves SKU/category/brand/model/variant/FOB/qty/cost/IVA. (~25 lines)
- [ ] 3.2 **GREEN** Add physical-file harness in `scripts/quality/spreadsheet-harness.js` with real parsers. (~80 lines)
- [ ] 3.3 **RED** Test: physical order route via `FileImporter` preserves fields + IVA semantics. (~20 lines)
- [ ] 3.4 **GREEN** Extend harness for order route; assert route identity per file. (~50 lines)
- [ ] 3.5 **RED** Test: wrong/silent route fails; parser error names file + field. (~15 lines)
- [ ] 3.6 **GREEN** Add route-assertion and error-naming in harness. (~30 lines)

## Slice 4: signed-updater-release-smoke (PR 4)

- [ ] 4.1 **RED** Test: without `TAURI_SIGNED_SMOKE=1` + manifest -> `SKIPPED_ENVIRONMENT_GATED`. (~10 lines)
- [ ] 4.2 **GREEN** Implement gate in `scripts/quality/updater-smoke.js`. (~25 lines)
- [ ] 4.3 **RED** Test: configured public key verification, placeholder rejection, metadata+sig+hash agreement. (~20 lines)
- [ ] 4.4 **GREEN** Implement ordered check->download->verify->install->restart->sentinel flow. (~100 lines)
- [ ] 4.5 **RED** Test: tampered artifact -> machine-readable rejection, old version active, sentinel unchanged. (~25 lines)
- [ ] 4.6 **GREEN** Implement tamper-rejection path. (~50 lines)

## Slice 5: catalog-image-storage (PR 5, AP-3a gated)

- [ ] 5.1 **RED** Test: read-only audit of inline images (missing/invalid/duplicate/orphan). (~25 lines)
- [ ] 5.2 **GREEN** Implement audit in `src/js/storage.js` producing `ImageRef{id,relativePath,mime,sha256,width,height}`. (~70 lines)
- [ ] 5.3 **RED** Test: migration creates backup + deterministic receipt; SKU change preserves image. (~20 lines)
- [ ] 5.4 **GREEN** Implement atomic migration: copy->resolve->commit(catalog+files+schema+receipt). Rollback on failure. (~100 lines)
- [ ] 5.5 **RED** Test: idempotent retry = no-op; orphans audit-visible, never auto-deleted. (~15 lines)
- [ ] 5.6 **GREEN** Implement idempotence + orphan audit. (~50 lines)
- [ ] 5.7 **GATE** AP-3a approval required before merge.

## Slice 6: historical-sku-audit-migration (PR 6, AP-3b gated)

- [ ] 6.1 **RED** Test: audit across catalog + `mambo_historial_v2` + selection; duplicate normalized identities get distinct SKUs. (~25 lines)
- [ ] 6.2 **GREEN** Implement audit + `SkuMapping{rowIdentity,oldSku,newSku}` in `src/js/skuAllocator.js`. (~80 lines)
- [ ] 6.3 **RED** Test: ambiguous mapping blocks migration before mutation. (~15 lines)
- [ ] 6.4 **GREEN** Implement ambiguity gate. (~25 lines)
- [ ] 6.5 **RED** Test: atomic commit updates 3 domains; image refs unchanged; receipt proves counts. (~20 lines)
- [ ] 6.6 **GREEN** Implement atomic commit + idempotent receipt + rollback. (~90 lines)
- [ ] 6.7 **GATE** AP-3b approval required before merge.

## Slice 7: ui-persistence-e2e (PR 7)

- [ ] 7.1 **RED** Test: fixture import shows traffic lights + R1-R10 details; RED not persisted; YELLOW/R9 reviewable. (~25 lines)
- [ ] 7.2 **GREEN** Wire fixture file-input + preview in `src/index.html` and `src/js/app.js`. (~80 lines)
- [ ] 7.3 **RED** Test: Tauri Store reload preserves catalog, selection, image, warnings. (~20 lines)
- [ ] 7.4 **GREEN** Verify Store persistence with disposable data. (~40 lines)
- [ ] 7.5 **RED** Test: Store-failure -> LocalStorage fallback -> identical recovery. (~20 lines)
- [ ] 7.6 **GREEN** Implement Store-failure detection + fallback in `src/js/storage.js`. (~60 lines)
