...
|---|---------|------|-----------|
| P1 | Extracción espacial PDF (grilla/filas) | 9 | FASE 2 CERRADA (20/20 slices: columnMap por headers, merged-cell por banda, matrices KZ/Haimu, anchor↔model). Gates frescos IT11 (2ª iteración): measure-model-quality FP_rate_clean 8% (2/25) SIN cambio; measure-extraction 44/65 mejoras / 0 OK-MENOR→CRITICO (vs 37/65 en gate FASE 2). 950/950 PASS. Deuda: pdfParser.js 2864 LOC, main thread |
| P2 | Sanitización de nombres/modelos | 10 | textSanitizer + tests (Transparent/High Resolution/SeaSalt), FP 8% estable; sobrevive IT1→IT7→IT11 (3+ iteraciones: 950/950 + audit PASS) |
...
| P4 | Asignación de imágenes | 9 | Pases 1-3 + gates: 0 cross-cat post-gates. **PASE 4 HÚNGARO POR DEFECTO (IT11, opt-out HUNGARIAN_P4=0)**: audit fresco G=2250 Y=64 R=0 vs baseline IT10 2247/67 → **+3 GREEN / −3 YELLOW**, 0 RED, 0 cross-cat, 0 dup, sin colgado (guard anti-loop, 8BitDo 1.4s IT6). Necesita 2ª iteración → 9 |
...
| P13 | Suite de tests | 9 | 950/950 (697+54+82+117) + suite build-frontend IT11 = 5 suites en runner oficial. Runner tocado hoy → 9 (2ª iteración en IT12) |
| P14 | Lint | 10 | **0 errores / 0 warnings repo-wide (IT12)**: limpiados los 56 heredados (35 no-useless-escape + 13 catch + 4 dead code + 2 scripts). Semántica preservada probada: audit G=2250 Y=64 R=0 idéntico pre/post, corpus 2314 IDÉNTICO con imágenes, gates FP 8% + 44/65 sin cambio |
...
| P17 | Build/optimización frontend | 9 | Opción 2 (lazy-load IT6: pdf.js 316K+worker 1.1MB + xlsx 864K fuera del arranque, head = papaparse 20K, 5 asserts) + **esbuild minify IT11**: scripts/build-frontend.js → dist/ espejo exacto (script tags INTACTOS, dev intacto), **463.784 → 243.136 bytes (−48%)**, pineado en el runner oficial. Necesita 2ª iteración → 9 |
...
Promedio: **9.8 (IT12, 18 procesos — P6 eliminado)**. Procesos >=9: **18/18** · **10: 14/18** (P2,P3,P5,P7,P8,P9,P10,P11,P12,P14,P15,P16,P18,P19). En 9: P1/P4/P17 (2ª iteración pendiente) + P13 (runner tocado hoy). Objetivo del loop: 18/18 en 10.
